...
from microworld.config import Config
...
from microworld.simulation.simulation_manager import SimulationManager, SimulationStatus
from microworld.simulation.simulation_runner import SimulationRunner, RunnerStatus
from microworld.telemetry.logging_config import get_logger
from microworld.domain.project import ProjectManager

logger = get_logger('microworld.api.simulation')


INTERVIEW_PROMPT_PREFIX = "Based on your persona, all past memories and actions, reply directly in text without using any tools: "
...
def create_simulation():
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Graph not yet built, please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        logger.debug(f"Checking simulation ready state: {simulation_id}, status={status}, config_generated={config_generated}")
        ...
        if status in prepared_statuses and config_generated:
            ...
            logger.info(f"Simulation {simulation_id} check result: ready (status={status}, config_generated={config_generated})")
            return True, {
                "status": status,
            ...
            logger.warning(f"Simulation {simulation_id} check result: not ready (status={status}, config_generated={config_generated})")
            return False, {
...
def prepare_simulation():
    import threading
    import os
    from microworld.domain.task import TaskManager, TaskStatus
    from microworld.config import Config
...
def get_prepare_status():
    from microworld.domain.task import TaskManager
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Not started yet, please call /api/simulation/prepare",
                        "already_prepared": False
                    }
                })
            return jsonify({
    ...
    try:
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enabling graph memory update requires a valid graph_id, please ensure the project has a built graph"
                }), 400
...
def interview_agent():
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Please ensure the simulation is complete and in command-wait mode."
            }), 400
