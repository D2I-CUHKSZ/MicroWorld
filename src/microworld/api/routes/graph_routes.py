...
from microworld.graph.ontology_generator import OntologyGenerator
from microworld.graph.graph_builder import GraphBuilderService
from microworld.ingestion.multimodal_ingestion import MultimodalIngestionService
from microworld.ingestion.text_processor import TextProcessor
from microworld.telemetry.logging_config import get_logger
from microworld.domain.task import TaskManager, TaskStatus
from microworld.domain.project import ProjectManager, ProjectStatus


logger = get_logger('microworld.api')
...
def generate_ontology():
    try:
        ...
        if not document_texts or not all_text.strip():
            ...
            return jsonify({
                "success": False,
                "error": "No usable text generated from multimodal input, please check file content"
            }), 400
...
def build_graph():
    try:
        ...
        def build_task():
            build_logger = get_logger('microworld.build')
            try:
                build_logger.info(f"[{task_id}] Starting graph construction...")
                ...
                build_logger.info(f"[{task_id}] Graph construction complete: graph_id={graph_id}, nodes={node_count}, edges={edge_count}")
